...
function dump_all($q) {
	echo "<pre style='float: left; border-left: 1px #444 solid;'>";
	echo "csets:";print_r($q->csets);
	echo "changes:";print_r($q->changes);
	echo "sets:";print_r($q->sets);
	echo "matches:";print_r($q->matches);
	echo "fsets:";print_r($q->fsets);
	echo "fields:";print_r($q->fields);
	echo "tsets:";print_r($q->tsets);
	echo "tables:";print_r($q->tables);
	echo "jsets:";print_r($q->jsets);
	echo "joints:";print_r($q->joints);
	echo "params:";print_r($q->params);	
	echo "list:";print_r($q->list);
	echo "datas:";print_r($q->datas);
	echo "datasets:";print_r($q->dsets);
	echo "<b>";
	echo $q->dump();
	echo "</b><BR>";
	echo "<b><u>";
...
$q->SELECT('*');
//..$q->WHERE(array('id'));//
//..$q->IN(array(20,30,40,50));
$q->WHERE(array('id'=>9));//, 'AND', array('id'=>7, 'age'=>8));
$q->DATA(array(5,'id'=>6,8));
//$q->WHERE(array('id'=>1, 'age'=>5));
//$q->WHERE('age < 3');
//$q->WHERE('OR');
//$q->WHERE('age < 4');
...
$q = null;
$q = new QRY();
$q->INTO('authors');//INSERT INTO authors (id) VALUES (?)
$q->INSERT('id');
$q->INSERT('age');
$q->VALUES(5);
...
$q = new QRY();
$q->FROM('mutants B, dragons D');
$q->FROM('goblins C');
$q->SELECT('*');
//..$q->WHERE(array('id'));//
//..$q->IN(array(20,30,40,50));
$q->WHERE('id');//, 'AND', array('id'=>7, 'age'=>8));
//$q->WHERE(array('id'=>1, 'age'=>5));
//$q->WHERE('age < 3');
//$q->WHERE('OR');
//$q->WHERE('age < 4');
...
$q->SELECT('title');
$q->SELECT('body');
$q->ON(array('id'=>'thread_id'));

//$q->FROM('users');			
//$q->SELECT('name');
//$q->ON(array('user_id'=>'id'));
